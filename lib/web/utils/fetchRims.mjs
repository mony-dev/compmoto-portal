...
const fetchRims = async () => {
  try {
    ...
    while (true) {
      ...
      // ดึงชื่อ Rim ไม่ซ้ำใน page นี้
      const rimNames = Array.from(
        new Set(
          products
            .map((p) => (p.Rim ? String(p.Rim).trim() : ''))
            .filter((name) => !!name),
        ),
      );

      if (rimNames.length > 0) {
        // หา rim ที่มีอยู่แล้วใน DB
        const existingRims = await prisma.rim.findMany({
          where: { name: { in: rimNames } },
          select: { name: true },
        });

        const existingSet = new Set(existingRims.map((r) => r.name));

        // เอาเฉพาะชื่อที่ยังไม่มีใน DB
        const missingNames = rimNames.filter(
          (name) => !existingSet.has(name),
        );

        console.log(
          `Found ${rimNames.length} unique rims in this page — ${missingNames.length} new`,
        );

        if (missingNames.length > 0) {
          const result = await prisma.rim.createMany({
            data: missingNames.map((name) => ({ name })),
            skipDuplicates: true,
          });

          totalInserted += result.count;
          console.log(`✓ Inserted ${result.count} new rims into DB`);
        }
      }
