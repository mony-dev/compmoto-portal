...
export default function adminSyncComrate({ params }: { params: { id: number } }) {
  ...
  async function fetchData(query: string = "") {
    setLoadPage(true);
    try {
      const { data } = await axios.get(`/api/adminComrate`, {
        params: {
          q: query,
          page: currentPage,
          pageSize: pageSize,
      ...
      const comrateDataWithKeys = data.data.map(
        (comrate: DataType, index: number) => ({
          ...comrate,
      ...
      >
        <div className="flex justify-between items-center">
          <div className="text-lg pb-4 default-font flex">
            <p className="text-lg font-semibold pb-4 grow">{t("Com rate")}</p>
          </div>
          <div className="flex">
            <Input.Search
