...
export default function AdminSyncSize({ params }: { params: { id: number } }) {
  ...
  async function fetchData(query: string = "") {
    setLoadPage(true);
    try {
      const { data } = await axios.get(`/api/adminSize`, {
        params: {
          q: query,
          page: currentPage,
          pageSize: pageSize,
      ...
      const sizeDataWithKeys = data.data.map(
        (size: DataType, index: number) => ({
          ...size,
          key: index + 1 + (currentPage - 1) * pageSize, // Ensuring unique keys across pages
        })
      );
      setSizeData(sizeDataWithKeys);
      setTotal(data.total);
    } catch (error: any) {
      console.log(error)
      toastError(error);
  ...
  const fetchSize = async () => {
    try {
      ...
      const { data } = await axios.post(`/api/fetchSize`);
      return data;
    } catch (error: any) {
      ...
      >
        <div className="flex justify-between items-center">
          <div className="text-lg pb-4 default-font flex">
            <p className="text-lg font-semibold pb-4 grow">{t("Size")}</p>
          </div>
          <div className="flex">
            <Input.Search
