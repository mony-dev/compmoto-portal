...
export default function adminSyncProductGroup({ params }: { params: { id: number } }) {
  ...
  const [productGroupData, setProductGroupData] = useState<DataType[]>([]);
  ...
  async function fetchData(query: string = "") {
    setLoadPage(true);
    try {
      const { data } = await axios.get(`/api/adminProductGroup`, {
        params: {
          q: query,
          page: currentPage,
          pageSize: pageSize,
      ...
      const productGroupDataWithKeys = data.data.map(
        (productGroup: DataType, index: number) => ({
          ...productGroup,
          key: index + 1 + (currentPage - 1) * pageSize, // Ensuring unique keys across pages
        })
      );
      setProductGroupData(productGroupDataWithKeys);
      setTotal(data.total);
    } catch (error: any) {
      console.log(error)
      toastError(error);
      ...
      >
        <div className="flex justify-between items-center">
          <div className="text-lg pb-4 default-font flex">
            <p className="text-lg font-semibold pb-4 grow">{t("Product Group")}</p>
          </div>
          <div className="flex">
            <Input.Search
