...
export default function AdminOrder({ params }: { params: { id: number } }) {
    ...
    try {
      const [invoiceResponse, orderResponse] = await Promise.all([
        ...
        axios.get(`/api/adminOrder`, {
          params: {
            q: query,
            type: "Normal", // ✅ keep Normal only here
            userId: session.user.id,
            role: session.user.role,
            page: currentPage,
            pageSize,
      ...
      // (ถ้าคุณใช้ total รวม แนะนำให้ใช้แยก: invoiceTotal/orderTotal มากกว่า)
  ...
  // ✅ single source of truth for fetching (debounced via timeout)
  useEffect(() => {
    if (!session?.user?.id) return;
    ...
    return () => {
      clearTimeout(timer);
      abortRef.current?.abort(); // stop inflight on dependency change/unmount
    };
