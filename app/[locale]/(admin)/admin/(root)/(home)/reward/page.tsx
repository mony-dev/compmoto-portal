  ...
  const fetchRewardUserData = async () => {
    try {
      const response = await axios.get(
        `/api/rewardUser?userId=${session?.user.id}`
      );
      if (response.data) {
        const rewardUser = response.data.map((data: RewardUserDataType) => ({
          id: data.id,
          rewardId: data.reward.id,
          name: data.reward.name,
          point: data.reward.point,
          date: formatDateRange(data.reward.startDate, data.reward.endDate),
  ...
  return (
    <div className="px-12">
      ...
      >
          ...
          <p className="leading-5 pt-2 ">
            ...
            <span className=" bg-comp-red-hover p-1 rounded">
              <span className="text-black">
                {`${session?.user?.rewardPoint}`} คะแนน
              </span>
            </span>
          </p>
        </div>
        ...
        <Divider orientation="left" style={{ borderColor: "#DD2C37" }}>
          <h1 className="text-3xl default-font">ประวัติการแลกรางวัล </h1>
        </Divider>
