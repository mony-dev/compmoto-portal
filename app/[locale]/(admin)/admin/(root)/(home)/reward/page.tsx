  ...
  async function fetchData() {
    ...
    try {
      const [redeemResponse] = await Promise.all([
        axios.get(`/api/rewardCategories`),
      ]);

      const userRewardWithKeys = redeemResponse.data.rewardCategories.map(
        (reward: DataType) => ({
          ...reward,
                  ...
                  <div className="grid gap-x-8 gap-y-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 py-4 reward-card">
                    {reward?.rewards.map((item, index) => (
                      ...
                      >
                        ...
                        <p className="text-sm default-font">{item.name}</p>
                                ...
                                render={({ field }) => (
                                  <div className="flex w-[160px] sm:w-[180px] custom-input-number">
                                    ...
                                    <Button
                                      ...
                                      onClick={() =>
                                        handleDecrement(`amount_${item.id}`)
                                      }
                                    />
                                    <Button
                                      className="bg-comp-disable-input border-none rounded-none plus-icon"
                                      icon={
                                        <PlusIcon className="w-4 text-comp-red" />
                                      }
                                      onClick={() =>
                                        handleIncrement(`amount_${item.id}`)
                                      }
                                    />
                                  </div>
                                )}
