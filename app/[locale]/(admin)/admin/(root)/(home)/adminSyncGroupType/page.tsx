...
export default function AdminSyncGroupType({ params }: { params: { id: number } }) {
  ...
  const [groupTypeData, setGroupTypeData] = useState<DataType[]>([]);
  ...
  async function fetchData(query: string = "") {
    setLoadPage(true);
    try {
      const { data } = await axios.get(`/api/adminGroupType`, {
        params: {
          q: query,
          page: currentPage,
          pageSize: pageSize,
      ...
      const groupTypeDataWithKeys = data.data.map(
        (groupType: DataType, index: number) => ({
          ...groupType,
          key: index + 1 + (currentPage - 1) * pageSize, // Ensuring unique keys across pages
        })
      );
      setGroupTypeData(groupTypeDataWithKeys);
      setTotal(data.total);
    } catch (error: any) {
      console.log(error)
      toastError(error);
  ...
  const fetchGroupType = async () => {
    try {
      ...
      const { data } = await axios.post(`/api/fetchGroupType`);
      return data;
    } catch (error: any) {
  ...
  const sync = async () => {
    try {
      ...
      await fetchGroupType(); // POST → enqueue
      ...
      >
        <div className="flex justify-between items-center">
          <div className="text-lg pb-4 default-font flex">
            <p className="text-lg font-semibold pb-4 grow">{t("Group Type")}</p>
          </div>
          <div className="flex">
            <Input.Search
